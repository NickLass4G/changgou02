...
public class AdminServiceImpl implements AdminService {
    ...
    public boolean login(Admin admin) {
        ...
        if (StringUtils.isBlank(admin.getLoginName())){
            ExceptionCast.cast(SystemCode.SYSTEM_ADMIN_LOGIN_USERNAME_BLANK_ERROR);
        }

        // 密码为空
        if (StringUtils.isBlank(admin.getPassword())){
            ExceptionCast.cast(SystemCode.SYSTEM_ADMIN_LOGIN_PASSWORD_BLANK_ERROR);
        }

        Admin argumentAdmin = new Admin();
        argumentAdmin.setLoginName(admin.getLoginName());
        argumentAdmin.setStatus("1");

        Admin adminResult = adminMapper.selectOne(argumentAdmin);
        if (adminResult == null){
            ExceptionCast.cast(SystemCode.SYSTEM_ADMIN_LOGIN_COUNT_ERROR);
        }

        // 校验密码
        boolean checkpw = BCrypt.checkpw(admin.getPassword(), adminResult.getPassword());
        if (!checkpw){
            ExceptionCast.cast(SystemCode.SYSTEM_ADMIN_LOGIN_ERROR);
        }
        return true;
    }
